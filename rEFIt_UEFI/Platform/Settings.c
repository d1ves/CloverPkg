                  ...
                  TagPtr   *Dict)
{
  ...
  ConfigPlistPath = PoolPrint (L"EFI\\XLoader\\%s.plist", ConfName);
...
*GetOtherKextsDir ()
{
  ...
  if (!FileExists (SelfVolume->RootDir, SrcDir)) {
    FreePool (SrcDir);
    SrcDir = PoolPrint (L"\\EFI\\XLoader\\kexts\\Other");
    if (!FileExists (SelfVolume->RootDir, SrcDir)) {
      FreePool (SrcDir);
      SrcDir = NULL;
    }
...
//    DBG("%a\n", FixedVersion);
  ...
  if (!FileExists (SelfVolume->RootDir, SrcDir)) {
    FreePool (SrcDir);
    SrcDir = PoolPrint (L"\\EFI\\XLoader\\kexts\\%a", FixedVersion);
    if (!FileExists (SelfVolume->RootDir, SrcDir)) {
      FreePool (SrcDir);
      SrcDir = NULL;
    }
