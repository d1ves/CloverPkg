...
// runtime debug
#define DBG_RT(entry, ...)    DebugLog(1, __VA_ARGS__)
...
VOID AppleRTCPatch(UINT8 *Driver, UINT32 DriverSize, CHAR8 *InfoPlist, UINT32 InfoPlistSize, LOADER_ENTRY *Entry)
{
  ...
  UINTN   NumMav = 0;
  //abbott >>
  DBG_RT(Entry, "\nabbott debug : AppleRTCPatch start", Driver, DriverSize);
  return;
  //abbott <<
...
VOID AnyKextPatch(UINT8 *Driver, UINT32 DriverSize, CHAR8 *InfoPlist, UINT32 InfoPlistSize, INT32 N, LOADER_ENTRY *Entry)
{
  ...
  if( AsciiStrCmp(Entry->KernelAndKextPatches->KextPatches[N].Label,"com.apple.driver.usb.AppleUSBXHCIPCI (change 15 port limit to 26 in XHCI kext)") == 0 ) return;
  if( AsciiStrCmp(Entry->KernelAndKextPatches->KextPatches[N].Label,"com.apple.driver.usb.AppleUSBXHCIPCI (NoLabel)") == 0 ) return;
   //abbott <<
  DBG_RT(Entry, "\nabbott test AnyKextPatch start\n");
