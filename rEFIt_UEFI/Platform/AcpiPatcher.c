...
{
  CHAR8 sign[5], OTID[9];
  ACPI_DROP_TABLE* DropTable = NULL;
...
VOID PatchAllTables()
{
  ...
  UINT32 Len = 0;
  EFI_PHYSICAL_ADDRESS BufferPtr = 0;
  EFI_STATUS Status = 0;
  EFI_ACPI_DESCRIPTION_HEADER* NewTable = 0;
  ...
  for (; Ptr < EndPtr; Ptr++) {
	...
	*/
    ...
    NewTable = (EFI_ACPI_DESCRIPTION_HEADER*)(UINTN)BufferPtr;
...
EFI_STATUS ReplaceOrInsertTable(VOID* TableEntry, UINTN Length, UINTN MatchIndex, INTN Pass)
{
  ...
  if(!EFI_ERROR(Status)) {
      EFI_ACPI_DESCRIPTION_HEADER* TableHeader = 0;
    Status = EFI_ABORTED;
    //DBG("page is allocated, write SSDT into\n");
    TableHeader = (EFI_ACPI_DESCRIPTION_HEADER*)(UINTN)BufferPtr;
    CopyMem(TableHeader, TableEntry, Length);
    // Now is a good time to fix the table header and checksum
    PatchTableHeader(TableHeader);
    FixChecksum(TableHeader);
...
void PreCleanupRSDT()
{
  ...
  Ptr = RsdtEntryPtrFromIndex(0);
  EndPtr = RsdtEntryPtrFromIndex(Count-1);
  for (; Ptr < EndPtr; Ptr++) {
    if (0 == Ptr[0] && 0 == Ptr[1]) {
...
void PostCleanupRSDT()
{
  ...
  Source = RsdtEntryPtrFromIndex(0);
  Dest = Source;
  EndPtr = RsdtEntryPtrFromIndex(Count);
  while (Source < EndPtr) {
    if (0 == *Source) {
...
VOID DumpTables(VOID *RsdPtrVoid, CHAR16 *DirName)
{
  ...
  if (Xsdt) {
    ...
    for (; Ptr < EndPtr; Ptr++) {
      ...
      Table = (EFI_ACPI_DESCRIPTION_HEADER*)(UINTN)ReadUnaligned64(Ptr);
      if (!Table) {
        DBG(" = 0\n");
        // skip NULL entry
        continue;
      ...
      if (EFI_ACPI_2_0_FIXED_ACPI_DESCRIPTION_TABLE_SIGNATURE == Table->Signature) {
        // Fadt - save Dsdt and Facs
          EFI_ACPI_2_0_FIXED_ACPI_DESCRIPTION_TABLE*  Fadt = 0;
...
VOID SaveOemTables()
{
  EFI_STATUS              Status;
  VOID                    *RsdPtr;
  CHAR16                  *AcpiOriginPath = PoolPrint(L"%s\\ACPI\\Dump", OEMPath);
  BOOLEAN                 Saved = FALSE;
  CHAR8                   *MemLogStart;
...
VOID SaveOemDsdt(BOOLEAN FullPatch)
{
  ...
  UINT8       *buffer = NULL;
  UINTN       DsdtLen = 0;
  CHAR16*     OriginDsdt = PoolPrint(L"%s\\ACPI\\Dump\\DSDT.aml", OEMPath);
  CHAR16*     OriginDsdtFixed = PoolPrint(L"%s\\ACPI\\Dump\\DSDT-%x.aml", OEMPath, gSettings.FixDsdt);
  CHAR16*     PathPatched   = L"\\EFI\\CLOVER\\ACPI\\Load";
  CHAR16*     PathDsdt;    //  = L"\\DSDT.aml";
//  CHAR16*     PathDsdtMini  = L"\\EFI\\CLOVER\\ACPI\\mini\\DSDT.aml";
  CHAR16*     AcpiOemPath = PoolPrint(L"%s\\ACPI\\Load", OEMPath);
        ...
        if (!ACPIPatchedAMLTmp) { // NULL when not disabled
          ...
          if (LoadPatchedAML(AcpiOemPath, gSettings.SortedACPI[Index], Pass)) {
            // avoid inserting table again on second pass
              ACPI_PATCHED_AML* temp2 = 0;
            for (temp2 = ACPIPatchedAML; temp2; temp2 = temp2->Next) {
...
//    DBG(" new len = 0x%x\n", bufferLen);
  ...
  if (gSettings.DebugDSDT) {
    ...
    Status = egSaveFile(SelfRootDir, L"\\EFI\\XLoader\\ACPI\\Dump\\DSDT-or.aml",
  ...
  if (gSettings.DebugDSDT) {
    for (Index=0; Index < 60; Index++) {
      ...
      UnicodeSPrint(DsdtPatchedName, 256, L"\\EFI\\XLoader\\ACPI\\Dump\\DSDT-pa%d.aml", Index);
...
EFI_STATUS PatchACPI_OtherOS(CHAR16* OsSubdir, BOOLEAN DropSSDT)
{
  ...
  if (FileExists(SelfRootDir, AcpiOemPath)) {
    ...
    FreePool(AcpiOemPath);
    PathPatched = PoolPrint(L"\\EFI\\XLoader\\ACPI\\%s", OsSubdir);
  }
