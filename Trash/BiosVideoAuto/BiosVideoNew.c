...
BIOS_VIDEO_DEV  *BiosVideoPublic;

CHAR8* readEDID()
{
  AsciiPrint("EdidDiscovered size=%d\n", BiosVideoPublic->EdidDiscovered.SizeOfEdid);
  if (BiosVideoPublic->EdidDiscovered.SizeOfEdid==0)
    return NULL;
  CHAR8* ret = AllocatePool(BiosVideoPublic->EdidDiscovered.SizeOfEdid);
  CopyMem(ret, BiosVideoPublic->EdidDiscovered.Edid, BiosVideoPublic->EdidDiscovered.SizeOfEdid);
  return ret;
}
...
/*  Msg = NULL;
  ...
  if ((Msg != NULL)&&(Status==EFI_SUCCESS)) {
    msgCursor = Msg->Cursor;
    BootLog("MsgLog Protocol installed in BiosVideo\n");
    Msg->Dirty = TRUE;
  }*/
...
/*		if (BiosVideoPrivate->EdidActive.Edid != NULL) {
		...
		BiosVideoPrivate->EdidActive.SizeOfEdid = LogSize;
		Status = gBS->AllocatePool (
									EfiBootServicesData,
									LogSize,
...
static BOOLEAN edid_checksum(UINT8 *edid)
{
	INTN i;
...
//----------------------------------------------------------------------------------
static const UINT8 edid_v1_header[] = { 0x00, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x00	};
static BOOLEAN edid_check_header(UINT8 *edid)
{
	INTN i; 
...
//------------------------------------------------------------------------
static BOOLEAN verifyEDID(UINT8 *edid)
{
...
/*		MsgLog("Timings: ");
	...
	BiosVideoPublic = BiosVideoPrivate;
  patchVideoBios();
  BiosVideoPublic = NULL;
	...
	VMP16 = (UINT16 *)&BiosVideoPrivate->VbeInformationBlock->VideoModePtr;
...
//	MsgLog("VbeModes: @%x\n", (UINTN)ModeNumberPtr);
    ...
    MsgLog("Unchecked mode 0x%x: %dx%d attr=%d\n",*ModeNumberPtr, BiosVideoPrivate->VbeModeInformationBlock->XResolution,
		...
		//
    if ((BiosVideoPrivate->VbeModeInformationBlock->BitsPerPixel!=24) && (BiosVideoPrivate->VbeModeInformationBlock->BitsPerPixel!=32)) {
      MsgLog("not 32-bit");
      continue;
    }
		...
		MsgLog("Successful mode 0x%x: %dx%d attr=%d\n",*ModeNumberPtr, BiosVideoPrivate->VbeModeInformationBlock->XResolution,
			   BiosVideoPrivate->VbeModeInformationBlock->YResolution,
			   BiosVideoPrivate->VbeModeInformationBlock->ModeAttributes);
